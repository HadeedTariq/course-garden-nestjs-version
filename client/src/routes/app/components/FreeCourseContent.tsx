...
const FreeCourseContent = ({ course }: FreeCourseContentProps) => {
  const navigate = useNavigate();
  const { isLoading, data } = useQuery({
    queryKey: ["coursePoints"],
    queryFn: async () => {
  ...
  const { isPending, mutate: enrollInCourse } = useMutation({
    mutationKey: ["enrollInFreeCourse"],
    mutationFn: async () => {
      const { data } = await studentApi.post(
        `/course/enroll/freeCourse?courseId=${course._id}`
      );
      return data;
    },
    onSuccess: () => {
      toast({
        title: "Successfully enrolled in free course",
      });
      navigate(
        `chapter?courseId=${course._id}&chapterId=${course.courseChapters[0]._id}`
      );
...
export default FreeCourseContent;
